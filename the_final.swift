...
class ViewController: UIViewController {
    ...
    func storeCurrNum() {
        ...
        guard let current = Int(inputLine.text!) else { print("Error storing current.")
            return }
        currentNum = current
        numEntries+=1
    }
    ...
    @IBAction func tappedEvaluate(){
        guard justTappedOP == true || enteringNumber || tappedEQL else { return print("ERROR EQL") }
        ...
        if pressedEq == false {
            ...
            case  .DIV:
                guard (currentNum != 0 && savedNum != 0) else {
                    HistoryLabel.text = "ERROR YOU BROKE THE UMIVERS"
                    return " "
                }
                savedNum /= currentNum
    ...
    //=======================================================================================================================
    @IBAction func LevyButton(_ sender: Any) {
        playYea()
        soundSet()
        didTapClear()
        clearHistory()
        inputLine.text = "Levy Rocks!"
    }
    
    func playYea(){
        do {
            let path = Bundle.main.path(forResource: "yeah.mp3", ofType:nil)!
